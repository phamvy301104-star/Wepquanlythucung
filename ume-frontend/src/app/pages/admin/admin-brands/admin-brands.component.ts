...
@Component({
  ...
  template: `
    <div class="page-header">
      <div><h4><i class="fas fa-tags text-gold"></i> Quản lý thương hiệu</h4>
        <ol class="breadcrumb"><li><a routerLink="/admin">Dashboard</a></li><li class="active">Thương hiệu</li></ol>
      </div>
      <button class="btn btn-gold" (click)="openForm()"><i class="fas fa-plus mr-1"></i>Thêm thương hiệu</button>
    </div>
    ...
    <div class="card">
      ...
      <div class="card-body p-0">
        <table class="adm-table">
          <thead><tr><th>#</th><th>Logo</th><th>Tên thương hiệu</th><th>Mô tả</th><th>Sản phẩm</th><th>Thứ tự</th><th>Trạng thái</th><th>Thao tác</th></tr></thead>
          <tbody>
            <tr *ngFor="let b of items; let i=index">
              <td>{{i+1}}</td>
              <td><img [src]="getImg(b.logoUrl||b.logo||b.imageUrl||b.image)" class="img-t-sm"></td>
              <td><strong>{{b.name}}</strong></td>
              <td class="muted">{{b.description?(b.description.length>50?b.description.substring(0,50)+'...':b.description):'-'}}</td>
              <td><span class="cat-b">{{b.productCount||0}}</span></td>
              <td>{{b.displayOrder||0}}</td>
              <td><label class="sw"><input type="checkbox" [checked]="b.isActive!==false" (change)="togAct(b)"><span class="sl"></span></label></td>
              <td><div class="act-g"><button class="ab aw" (click)="openForm(b)"><i class="fas fa-edit"></i></button><button class="ab ar" (click)="confirmDel(b)"><i class="fas fa-trash"></i></button></div></td>
            </tr>
            <tr *ngIf="!items.length&&!ld"><td colspan="8" class="empty">Không có thương hiệu nào</td></tr>
    ...
    <div class="mo" *ngIf="showForm" (click)="showForm=false"><div class="md" (click)="$event.stopPropagation()">
      <div class="mh bg-g"><h5><i class="fas fa-tags mr-2"></i>{{editing?'Sửa':'Thêm'}} thương hiệu</h5><button class="mx" (click)="showForm=false">&times;</button></div>
      <div class="mb-modal">
        <div class="fg"><label>Tên thương hiệu <span class="req">*</span></label><input class="form-control" [(ngModel)]="fd.name"></div>
        <div class="fg"><label>Mô tả</label><textarea class="form-control" [(ngModel)]="fd.description" rows="3"></textarea></div>
        <div class="fg"><label>Thứ tự</label><input type="number" class="form-control" [(ngModel)]="fd.displayOrder" min="0"></div>
        <div class="fg"><label>Logo</label><div class="img-up-sm" (click)="fi.click()"><img *ngIf="imgPv" [src]="imgPv" class="pv-img"><div *ngIf="!imgPv" class="up-ph"><i class="fas fa-image"></i><span>Chọn ảnh</span></div></div><input type="file" #fi (change)="onFile($event)" accept="image/*" hidden></div>
    ...
    <div class="mo" *ngIf="showDel" (click)="showDel=false"><div class="md" (click)="$event.stopPropagation()">
      ...
      <div class="mb-modal"><p>Bạn có chắc muốn xóa <strong>{{delItem?.name}}</strong>?</p></div>
  ...
  styles: [`
    ...
    .card-header{background:#fff;border-bottom:1px solid #f0f0f0;padding:12px 16px;border-radius:12px 12px 0 0!important;display:flex;justify-content:space-between;align-items:center}.card-body{padding:16px}
    ...
    .img-t-sm{width:50px;height:50px;object-fit:contain;border-radius:6px;border:1px solid #dee2e6}
    .cat-b{background:#17a2b8;color:#fff;padding:2px 8px;border-radius:4px;font-size:.75rem}.muted{color:#6c757d}
    ...
    .img-up-sm{width:100%;height:120px;border:2px dashed #dee2e6;border-radius:8px;cursor:pointer;display:flex;align-items:center;justify-content:center;overflow:hidden}.img-up-sm:hover{border-color:#D4AF37}
    .pv-img{width:100%;height:100%;object-fit:cover}.up-ph{display:flex;flex-direction:column;align-items:center;color:#adb5bd;gap:8px}.up-ph i{font-size:1.5rem}
...
export class AdminBrandsComponent implements OnInit {
  ...
  load() {
    this.ld = true;
    this.api.getBrands({ limit: 100, search: this.search || undefined }).subscribe({
      next: (r: any) => { this.ld = false; this.items = r.data?.brands || r.data || []; },
      error: () => this.ld = false
    });
  }
  ...
  openForm(b?: any) {
    this.editing = b || null;
    this.fd = b ? { name: b.name, description: b.description || '', displayOrder: b.displayOrder || 0, isActive: b.isActive !== false }
      : { name: '', description: '', displayOrder: 0, isActive: true };
    this.selFile = null; this.imgPv = this.getImg(b?.logoUrl || b?.logo || b?.imageUrl || b?.image || '');
    this.showForm = true;
  }
  ...
  save() {
    ...
    if (this.selFile) d.append('logo', this.selFile);
    const obs = this.editing ? this.api.updateBrand(this.editing._id, d) : this.api.createBrand(d);
  ...
  togAct(b: any) { const d = new FormData(); d.append('isActive', String(!b.isActive)); this.api.updateBrand(b._id, d).subscribe({ next: () => b.isActive = !b.isActive, error: () => {} }); }
  confirmDel(b: any) { this.delItem = b; this.showDel = true; }
  doDelete() { if (!this.delItem) return; this.api.deleteBrand(this.delItem._id).subscribe({ next: () => { this.msg = 'Đã xóa!'; this.load(); this.showDel = false; setTimeout(() => this.msg = '', 3000); }, error: () => this.showDel = false }); }
}
